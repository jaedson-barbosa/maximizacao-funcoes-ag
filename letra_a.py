...
def plotar(nome: str, avaliacoes: list[float], melhores_x: list[float]):
    ...
    plt.figure()
    plt.subplot(1, 2, 1)
    plt.plot(geracoes, avaliacoes, 'r--')
    plt.xlim((1, numero_de_geracoes))
    plt.yscale('log')
    plt.xlabel('Gerações')
    plt.ylabel('Fitness')
...
def cruzamento(bits_povo: list[list[int]], indices: list[int]) -> list[list[int]]:
    indices = [i for i in indices if random.random() < prob_cruzamento]
...
main('funcao-letra-a-roleta', selecao_roleta)
main('funcao-letra-a-torneio', selecao_torneio)
